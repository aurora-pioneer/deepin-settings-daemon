...
set_output_info (output_t *output, RROutput xid, XRROutputInfo *output_info)
{
	...
	if (!output->crtc_info)
	{
		...
		printf ("cannot find crtc 0x%x\n", output->crtc.xid);
	    if (output->crtc.kind & name_index)
		printf ("cannot find crtc %d\n", output->crtc.index);
	}
	if (!output_can_use_crtc (output, output->crtc_info))
	    printf ("output %s cannot use crtc 0x%x\n", output->output.string,
	...
	if (output->mode.xid)
	{
	    ...
	    if (!output->mode_info)
		printf ("server did not report mode 0x%x for output %s\n",
	...
	printf ("output %s cannot use rotation \"%s\" reflection \"%s\"\n",
	       output->output.string,
...
{
    if (!has_1_2)
        printf ("Server RandR version before 1.2\n");
    ...
    if (!res) printf ("could not get screen resources");
}
...
get_crtcs (void)
{
	...
	if (!crtc_info) printf ("could not get crtc 0x%x information\n", res->crtcs[c]);
...
crtc_add_output (crtc_t *crtc, output_t *output)
{
    ...
    if (!crtc->outputs) printf ("out of memory\n");
...
static int set_gamma()
{
	...
	if (!output->crtc_info) {
	    printf("Need crtc to set gamma on.\n");
	    return -1;
	}
	...
	if (!size) {
	    printf("Gamma size is 0.\n");
	    continue;
	}

	gamma = XRRAllocGamma(size);
	if (!gamma) {
	    printf("Gamma allocation failed.\n");
	    continue;
	}
...
crtc_apply (crtc_t *crtc)
{
	...
	if (s == RRSetConfigSuccess && crtc->panning_info) {
		...
		printf ("panning needs RandR 1.3\n");
	}
    }
    free (rr_outputs);
    return s;
...
get_outputs (void)
{
	...
	output_t	*output;
	name_t		output_name;
	if (!output_info) printf ("could not get output 0x%x information\n", res->outputs[o]);
...
mark_changing_crtcs (void)
{
	...
	for (o = 0; o < crtc->crtc_info->noutput; o++)
	{
	    ...
	    if (!output) printf ("cannot find output 0x%x\n",
...
find_crtc_for_output (output_t *output)
{
	...
	if (!crtc) printf ("cannot find crtc 0x%x\n", output->output_info->crtcs[c]);
...
set_positions (void)
{
	...
	for (output = outputs; output; output = output->next)
	{
	    ...
	    if (!relation) printf ("cannot find output \"%s\"\n", output->relative_to);
	...
	if (!keep_going)
	    break;
	if (!any_set)
	    printf ("loop in relative position specifications\n");
    }
...
set_screen_size (void)
{
    ...
    if (fb_width > maxWidth || fb_height > maxHeight)
        printf ("screen cannot be larger than %dx%d (desired size %dx%d)\n",
...
pick_crtcs (void)
{
	...
	if (output->mode_info && !output->crtc_info)
	    printf ("cannot find crtc for output %s\n", output->output.string);
...
#define ModeShown   0x80000000
    ...
    if (m_output_names) 
        memset(m_output_names, 0, res->noutput + 1);
    /* TODO: travel the outputs */
    int index;
    for (output = outputs, index = 0; output && index < res->noutput; output = output->next, index++)
	{
        XRROutputInfo   *output_info = output->output_info;
	    ...
	    if (m_output_names) {
            ...
            if (m_output_names[index]) {
                ...
                if (output_info->connection) 
                    ...
                    strcpy(m_output_names[index], output_info->name);
            }
        }
        if (mode)
	    {
		...
		if (!mode_shown) printf ("out of memory\n");
        for (j = 0; j < output_info->nmode; j++)
		{
