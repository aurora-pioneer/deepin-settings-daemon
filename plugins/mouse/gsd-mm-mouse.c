...
mouse_init_settings (GsdMouseManager *manager)
{
    ...
    mouse_set_tweaks_daemon (manager,
...
mouse_apply_settings (GsdMouseManager *manager, GdkDevice *device)
{
    ...
    mouse_set_middle_button (manager, device, g_settings_get_boolean (manager->priv->mouse_settings, KEY_MIDDLE_BUTTON_EMULATION));
}

static void
mouse_set_middle_button (GsdMouseManager *manager, GdkDevice *device, gboolean middle_button)
{
    Atom prop;
    XDevice *xdevice;
    Atom type;
...
void
mouse_set_tweaks_daemon (GsdMouseManager *manager, gboolean dwell_click_enabled, gboolean secondary_click_enabled)
{
    GError *error = NULL;
    gchar *comm;
    ...
    if (run_daemon || manager->priv->mousetweaks_daemon_running)
        comm = g_strdup_printf ("mousetweaks %s", run_daemon ? "" : "-s");
    else
        return;
    ...
    if (! g_spawn_command_line_async (comm, &error))
    {
        if (error->code == G_SPAWN_ERROR_NOENT && run_daemon)
        {
            ...
            {
                g_settings_set_boolean (manager->priv->mouse_a11y_settings, KEY_DWELL_CLICK_ENABLED, FALSE);
            }
            else if (secondary_click_enabled)
            {
                g_settings_set_boolean (manager->priv->mouse_a11y_settings, KEY_SECONDARY_CLICK_ENABLED, FALSE);
            }
...
void
mouse_callback (GSettings *settings, const gchar *key, GsdMouseManager *manager)
{
    ...
    for (l = devices; l != NULL; l = l->next)
    {
        ...
        {
            mouse_set_middle_button (manager, device, g_settings_get_boolean (settings, KEY_MIDDLE_BUTTON_EMULATION));
        }
    }
    g_list_free (devices);
}
